...
/**
 * FileChooserDialog that can load a board when the file was selected or save a board into that file
 */
class SaveLoadDialog(private val parent: Window) : JFileChooser() {
    ...
    private fun setFilters() {
        val golFilter = FileNameExtensionFilter(
                GuiResources.default.getStringOrKey(GuiResources.FILE_FILTER_KEY),
                FILE_EXTENSION
        )
    ...
    /**
     * Shows the save dialog and tries to save [board] into the selected file. If an exception is thrown,
     * a error message is shown to the user.
     *
     * @param board the board that should be saved into the selected file
     */
    fun saveBoard(board: Board) {
        ...
        try {
            ...
            showErrorMessage(
                    GuiResources.default.getStringOrKey(GuiResources.FILE_SAVE_ERROR_GENERAL_KEY),
    ...
    /**
     * Shows the load dialog and tries to load a [Board] from the selected file. If an exception is thrown,
     * a error message is shown to the user.
     */
    fun loadBoard(): Board? {
        ...
        return try {
            ...
            showErrorMessage(
                    GuiResources.default.getFormattedStringOrKey(
                            GuiResources.FILE_LOAD_ERROR_FORMAT_KEY,
                            e.localizedMessage),
            ...
            showErrorMessage(
                    GuiResources.default.getStringOrKey(GuiResources.FILE_LOAD_ERROR_GENERAL_KEY),
                    GuiResources.default.getStringOrKey(GuiResources.FILE_LOAD_ERROR_TITLE_KEY)
            )
            null
        }
